var Graficos = (function () {
        ...
        switch (elemento.ConteudoPrimario) {
            ...
            case 'filtro':
                if (elemento.ConteudoSecundario == 'dropdown') {
                    ...
                    return `<div class="item filtro" data-idelemento="${elemento.IDElemento}">
                            <div class="box-floating">
                                <select id="${elemento.IDComponente}" class="floating-select" data-concatena="${elemento.ConcatenaFiltros}" onchange="Graficos.ddlChange('${elemento.IDComponente}')" >
                                    <option value="0">Carregando...</option>
                                </select>
                                <label class="label-floating">${elemento.Titulo}</label>
                            </div>
            ...
            case 'custom':
                return `<div class="item" data-idelemento="${elemento.IDElemento}" id="${elemento.IDComponente}" ></div>`;
            case 'table':
        ...
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < arrElementosParaAtualizar.length; i++) {
                if (Loaders[arrElementosParaAtualizar[i].ConteudoPrimario]) {
                    ...
                    //} catch (e) {
                    //    console.log(arrElementosParaAtualizar[i]);
                    //    console.warn(`Dashboard - Erro ao renderizar gráfico - ${arrElementosParaAtualizar[i].IDComponente}`);
                    //    Graficos.renderizaMockupErro(arrElementosParaAtualizar[i]);
                    //    console.warn(e);
                    //}                    
                }
            }
            ...
            Promise.all(arrPromises).then(() => {
                ...
                console.error(`Dashboard - Erro ao carregar um dos elementos da página. Analisar console para mais detalhes`);                
            });
        });
    }
    ...
    function configurarFloatingLabelsSingle(IDComponente) {
        return new Promise((resolve, reject) => {
            //$('.floating-select').attr('value', '');
            $(`#${IDComponente}`).on('change', function () { RefreshFloatingLabel(this); });
            $(`#${IDComponente}`).on('focusout', function () { RefreshFloatingLabel(this); });
            $(`#${IDComponente}`).on('mouseover', function () { RefreshFloatingLabel(this); });
            $(`#${IDComponente}`).on('mouseout', function () { RefreshFloatingLabel(this); });
                        
            $(`#${IDComponente}`).on('input', function () { RefreshFloatingLabel(this); });
            $(`#${IDComponente}`).on('focusout', function () { RefreshFloatingLabel(this); });
            $(`#${IDComponente}`).on('mouseover', function () { RefreshFloatingLabel(this); });
            $(`#${IDComponente}`).on('mouseout', function () { RefreshFloatingLabel(this); });
            resolve();
        ...
        return new Promise((resolve, reject) => {
            ...
            Dash_Util.invokeVanilla(Dashboard_Actions.ExecuteDynamicAsync, 'POST', 'application/json', dataDynamic, true, function (resultDynamic) {
                if (resultDynamic !== null && resultDynamic.length > 0) {
                    ...
                    renderizaMockupErro(elemento);
                    console.warn(`[BD]Dashboard - Nenhum item encontrado - ${elemento.IDComponente}`);
                    reject();
                }
            }, (errResponse) => {
                console.warn(`[BD]Dashboard - Erro ao executar elemento - ${elemento.IDComponente} - ${errResponse}`);
                renderizaMockupErro(elemento);
                reject();
            });
        });
    ...
    function renderizaMockupErro(elemento) {
        let html = '';
        const elementoRenderizado = document.getElementById(`${elemento.IDComponente}`);
        const boxElementoRenderizado = document.getElementById(`${Graficos.boxPrefix}${elemento.IDComponente}`);
        ...
        switch (elemento.ConteudoPrimario) {
            ...
            case 'filtro':
                html = '<option value="0">Erro ao carregar...</option>';                
                elementoRenderizado.innerHTML = html;                
                break;
            ...
            case 'chart':                
                html = `<div>
                        ...
                        <div class="fa-3x loading text-center" style="flex-grow: 1;">
                            <i class="fas fa-exclamation-triangle"></i>
                            <p>Erro ao carregar</p>
                        
                        </div>`;
                boxElementoRenderizado.innerHTML = html;
                break;

            case 'custom':
                html = `<div class="fa-3x loading text-center" style="flex-grow: 1;">
                            <i class="fas fa-exclamation-triangle"></i>
                            <p>Erro ao carregar</p>                        
                        </div>`;
    ...
    return {
        ...
        renderizaMockupErro,
        configurarFloatingLabelsSingle
    };
