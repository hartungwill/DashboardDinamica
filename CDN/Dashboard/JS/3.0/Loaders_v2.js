var Loaders = (function () {
    ...
    const chart = function (elemento) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                renderChartGerador(`${Graficos.boxPrefix}${elemento.IDComponente}`, resultDynamic, elemento, null);
    ...
    function renderChartGerador(idBoxDiv, resultDynamic, elemento, customDataset) {
        if (elemento.Html) {
            document.getElementById(`${idBoxDiv}`).innerHTML += `<div><h5 class="titulo">${elemento.Titulo}</h5></div><canvas style="${elemento.Html}" id="${elemento.IDComponente}"></canvas><div id=${elemento.IDComponente}-Legenda class="chart-legenda"></div>`;
        } else {
            document.getElementById(`${idBoxDiv}`).innerHTML += `<div><h5 class="titulo">${elemento.Titulo}</h5></div><div class="box-canvas"><canvas id="${elemento.IDComponente}"></canvas></div><div id=${elemento.IDComponente}-Legenda class="chart-legenda"></div>`;
        }
        ...
        let options = window["Loaders"][`${elemento.ConteudoSecundario}Config`](resultDynamic, elemento, customDataset);

        //Renderiza o gráfico com o contexto e as configurações
        let chart = new Chart(ctx, options);

        //Renderiza legenda customizada
        renderLegendaGraficosCustom(idBoxDiv, elemento.IDComponente, options);

        let Box = document.querySelector(`${'#' + idBoxDiv}`);
        Box.querySelector('.fa-3x').remove();
    }

    function renderChart(idBoxDiv, idComponenteCanvas, titulo, options) {
        document.getElementById(`${idBoxDiv}`).innerHTML = `<div><h5 class="titulo">${titulo}</h5></div><div class="box-canvas"><canvas id="${idComponenteCanvas}"></canvas></div><div id=${idComponenteCanvas}-Legenda class="chart-legenda"></div>`;

        let ctx = document.getElementById(idComponenteCanvas);

        //Renderiza o gráfico com o contexto e as configurações
        let chart = new Chart(ctx, options);

        //Renderiza legenda customizada
        renderLegendaGraficos(chart, idBoxDiv, idComponenteCanvas);
    }
    ...
    function renderLegendaGraficos(chart, idBoxDiv, idComponenteCanvas) {
        $(`#${idBoxDiv}`).find(`#${idComponenteCanvas}-Legenda`).append(chart.generateLegend());
    }

    function renderLegendaGraficosCustom(idBoxDiv, idComponenteCanvas, options) {
        $(`#${idBoxDiv}`).find(`#${idComponenteCanvas}-Legenda`).append(getLegendHTML(options));
    }

    function getLegendHTML(options) {
        let ListaGrafico = options.data.datasets;
        let html = '';
        var bar = '';
        var linha = '';
        var doughnut = '';
        ...
        for (let i = 0; i < ListaGrafico.length; i++) {
            if (ListaGrafico[i].type == 'bar') {
                bar += `<li><span style="background-color: ${ListaGrafico[i].legendColor};"></span>${ListaGrafico[i].label[0]}</li>`
            } else if (ListaGrafico[i].type == 'line') {
                linha += `<li><span style="background-color: ${ListaGrafico[i].legendColor}; width: 35px; height: 4px;"></span>${ListaGrafico[i].label[0]}</li>`
            } else if (options.type === 'doughnut') {
                doughnut += ListaGrafico[i].legenda.map((legenda, indexlegenda) => `<li><span style="background-color: ${legenda.backgroundColor};"></span>${legenda.key}</li>`).join('');
            }
        }

        html += `<ul class="0-legend">
                          ${bar}
                          ${linha}
                          ${doughnut}
                     </ul>`
    ...
    function getDataset(resultDynamic, conteudoSecundario, baseDataSet = null) {
        let chartData = {
            labels: [],
            datasets: {}
        };
        ...
        for (let i = 0; i < resultDynamic.length; i++) { // Linhas
            ...
            if (baseDataSet) {
                chartData.datasets[i] = new baseDataSet;
            } else {
                chartData.datasets[i] = newDataSet(conteudoSecundario);
            }
                ...
                switch (coluna) {
                    ...
                    default: // Dados
                        if ((valorAtual === 'line' || valorAtual === 'bar')) { // Tipo do DataSet para os gráficos agrupados
                            ...
                            if (conteudoSecundario == 'stacked' || conteudoSecundario == 'stackedGroup') {
                                if (valorAtual === 'line') {
                            ...
                            const data = trataValor(valorAtual);
                            const dataReal = trataValorReal(valorAtual);

                            chartData.datasets[i].data.push(data);
                            chartData.datasets[i].dataReal.push(dataReal);
                        }
                        break;
                }
        ...
        if (valor !== undefined && valor !== '') {
            if (isInt(valor)) {
                ...
                if (valorTratado.includes('|')) { // troca de | (pipe) por vírgula(separar array)
                    ...
                    valorTratado = valorTratado.map(Function.prototype.call, String.prototype.trim); //remove espaços
                }

                if (valorTratado.includes(';')) { // troca de ; (ponto e vírgula) por vírgula(separa array)
                    valorTratado = valorTratado.split(';');
                    valorTratado = valorTratado.map(Function.prototype.call, String.prototype.trim); //remove espaços
                    valorTratado = valorTratado[0]; // sempre o valor antes do ';'                    
                }
    ...
    function trataValorReal(valorReal) {
        if (valorReal !== undefined && valorReal !== '') {
            if (isInt(valorReal)) {
                ...
                if (valorTratado.includes('|')) { // troca de | (pipe) por vírgula(separa array)
                    valorTratado = valorTratado.split('|');
                    valorTratado = valorTratado.map(Function.prototype.call, String.prototype.trim); //remove espaços
                } else if (valorTratado.includes(';')) { // troca de ; (ponto e vírgula) por vírgula(separa array)
                    valorTratado = valorTratado.split(';');
                    valorTratado = valorTratado.map(Function.prototype.call, String.prototype.trim); //remove espaços
                    valorTratado = valorTratado[1]; // sempre o valor após o ';'                    
                }
    ...
    function pieConfig(resultDynamic, elemento, customDataset) {
        let arrData = [];
        let arrLabels = [];
        let arrCorFonte = [];
        let arrCorFundo = [];
    ...
    function lineConfig(resultDynamic, elemento, customDataset) {
        let lineBarChartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
    ...
    function horizontalBarConfig(resultDynamic, elemento, customDataset) {
        let horizontalBarChartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
    ...
    function barConfig(resultDynamic, elemento, customDataset) {
        let barChartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
    ...
    function customConfig(resultDynamic, elemento, customDataset) {
        let barChartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
    ...
    function stackedConfig(resultDynamic, elemento, customDataset) {
        let chartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
        //chartData.datasets = Object.keys(chartData.datasets).map(i => chartData.datasets[i]);
    ...
    function stackedGroupConfig(resultDynamic, elemento, customDataset) {
        let chartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
        //chartData.datasets = Object.keys(chartData.datasets).map(i => chartData.datasets[i]);
    ...
    function doughnutConfig(resultDynamic, elemento, customDataset) {
        let valorAtual;
        let colunaAtual;
        let colunas = [];
            ...
            for (let coluna = 0; coluna < resultDynamic[j].length; coluna++) { // Colunas
                ...
                switch (coluna) {
                    ...
                    case 1: // Cor Primária: ponto, fonte
                        ...
                        donutBarChartData.datasets[0].fontColor = valorAtual;
                        break;
                    ...
                    case 3: // Cor Ternária: Box da legenda - datalabels
                        donutBarChartData.datasets[0].legendColor = valorAtual;
                        break;
        ...
        if (typeof getCustomConfig === "function") {
            return getCustomConfig(donutBarChartData, elemento, elemento.ConteudoSecundario);
        } else {
            return getCommonConfig(donutBarChartData, elemento, elemento.ConteudoSecundario);
        }
    }

    function doughnutCustomConfig(resultDynamic, elemento, customDataset) {
        return doughnutConfig(resultDynamic, elemento);
    }

    function waterfallConfig(resultDynamic, elemento, customDataset) {
        let barChartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
        //barChartData.datasets = Object.keys(barChartData.datasets).map(i => barChartData.datasets[i]);
    ...
    function deltaConfig(resultDynamic, elemento, customDataset) {
        let barChartData = getDataset(resultDynamic, elemento.ConteudoSecundario, customDataset);
        //console.log('DELTA');
                        ...
                        tooltips: {
                            callbacks: {
                                label: (tooltipItem, data) => {
                                    ...
                                    return label + ': ' + String(Array.isArray(v) ? v[1] - v[0] : v);
                                }
                            }
                        },
                        hover: {
